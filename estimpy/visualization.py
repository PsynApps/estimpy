...
class VideoVisualization(Visualization):
    ...
    def _dr_osc_extract_waveform(self, channel_id, current_time):
        ...
        Duration is determined by manual override or the current auto mode (tone or pulse).
        Uses cross-correlation with the previous frame's waveform for smooth tracking,
        falling back to zero-crossing trigger when no template exists or correlation
        quality is too low."""
        ...
        trigger_search_end = min(len(buffer), len(buffer) - display_samples)
        if trigger_search_end <= 0:
        ...
        if prev_template is not None and len(prev_template) == display_samples:
            ...
            if corr_quality > 0.3:
                ...
                trigger_idx = self._dr_osc_find_trigger_zero_crossing(buffer[:trigger_search_end])
        else:
            # No previous template or length changed — use zero-crossing
            trigger_idx = self._dr_osc_find_trigger_zero_crossing(buffer[:trigger_search_end])

        # Clamp to valid range
        end_idx = trigger_idx + display_samples
        if end_idx > len(buffer):
            end_idx = len(buffer)
            trigger_idx = max(0, end_idx - display_samples)
